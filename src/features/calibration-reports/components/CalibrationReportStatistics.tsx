...
import { CalibrationReport, CalibrationStatus } from "../types";
...
export default function CalibrationReportStatistics({
  ...
  const statistics = useMemo(() => {
    ...
    const passed = reports.filter(
      (report) => report.status === CalibrationStatus.PASS
    ).length;

    const failed = reports.filter(
      (report) => report.status === CalibrationStatus.FAIL
    ).length;

    const pending = reports.filter(
      (report) => report.status === CalibrationStatus.PENDING
    ).length;
